...
% ------------------------------------------------------------------------
function [ucm2, ucms, elapsed_time] = img2ucms(I, edges, model, scales, all_parameters, param_multi, align_thr)
...
for s = 1:numel(scales),
    ...
    [ucms{s}, times] = img2ucm_scale(I, edges, model, param);
...
%%
function [ucm2, times] = img2ucm_scale(I, edges, model, param)
% compute hierarchical segmentation at each scale independently
I = imresize(I, param.scale, 'lanczos3');
[ucm2, times] = img2ucm(I, edges, model, param.mult_Pb, param.sat_sPb, param.nvec, param.dthresh, param.ic_gamma);
...
%%
function [ucm2, times] = img2ucm(I, edges, model, mult_Pb, sat_sPb, nvec, dthresh, ic_gamma)
...
subplot(2,2,1),imagesc(E);
subplot(2,2,2),imagesc(O);
subplot(2,2,3),imagesc(edges{1});
subplot(2,2,4),imagesc(edges{2});
pause;
end
